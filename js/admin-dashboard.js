...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    const bookingsEl = document.getElementById('total-bookings-display');
    const usersEl = document.getElementById('active-users-display');
    const inquiriesEl = document.getElementById('pending-inquiries-display');

    if (revenueEl) revenueEl.textContent = '$' + stats.revenue.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 });
    if (bookingsEl) bookingsEl.textContent = stats.total_bookings;
    if (usersEl) usersEl.textContent = stats.active_users;
    if (inquiriesEl) inquiriesEl.textContent = stats.pending_inquiries;
    ...
    // --- Sidebar Toggle Logic (Shared across all admin pages) ---
    ...
    // Run once on load to ensure proper margin
